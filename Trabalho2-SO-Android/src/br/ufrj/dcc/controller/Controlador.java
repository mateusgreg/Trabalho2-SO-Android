...
public class Controlador {
	...
	private Controlador(){
		this.reiniciaDependencias();
	}
	
	private void reiniciaDependencias(){
	...
	public void iniciaPrograma(){
		...
		for(int i = 0; i < Constantes.N_PRODUTORES; i++){
			atores[i] = new Produtor(this.buffer, this.gerenteRecurso, this.bufferCheioSemaforo, this.temRecursoBufferSemaforo, this.mutex);
		}
		
		for(int i = Constantes.N_PRODUTORES; i < Constantes.N_PRODUTORES + Constantes.N_CONSUMIDORES; i++){
			atores[i] = new Consumidor(this.buffer, this.gerenteRecurso, this.bufferCheioSemaforo, this.temRecursoBufferSemaforo, this.mutex);
		}
